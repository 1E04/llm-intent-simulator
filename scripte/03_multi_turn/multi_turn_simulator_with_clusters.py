"""Multi-Turn Simulator - Human Base Input Variant (With Anti-Drift Clustering)

Same simulation loop as ``multi-turn-simulator.py``, with one difference:
there are **no personas**.
It uses sentence_transformers clustering to provide anti-drift instructions.
"""
...
# Import the new intent clustering helper
...
_core_spec.loader.exec_module(core_module)
...
class BaseInputUserSimulator(UserSimulator):
    ...
    def generate_user_turn(
        ...
        if not BaseInputUserSimulator._prompt_printed:
            with BaseInputUserSimulator._print_lock:
                if not BaseInputUserSimulator._prompt_printed:
                    print("\n" + "="*80)
                    print("=== EXAMPLE SIMULATOR SYSTEM PROMPT (First Generated Turn) ===")
                    print("="*80)
                    
                    # Fill the remaining placeholders just for the printout so it matches the real final prompt
                    display_prompt = self.profile_data["simulator_system_prompt"].replace(
                        "{target_intent}", str(target_intent)
                    ).replace(
                        "{forbidden_label_text}", str(target_intent).replace("_", " ")
                    )
                    
                    print(display_prompt)
                    print("="*80 + "\n")
                    BaseInputUserSimulator._prompt_printed = True
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Multi-Turn Simulator that starts from the human base test CSV (with clustering anti-drift).")
    ...
    parser.add_argument("--num_dialogues", type=int, default=5,
                        help="Number of dialogues (one real CSV utterance each). Use 0 or -1 for the ENTIRE dataset. Ignored with --per_intent.")
    parser.add_argument("--sample_mode", choices=["random", "sequential", "per_intent"],
                        default="random",
                        help="random: seeded random subset | sequential: CSV order from --start_index | per_intent: --per_intent rows per intent.")
    ...
    print("=" * 70)
    print(" MULTI-TURN SIMULATION (HUMAN BASE INPUT, NO PERSONAS, ANTI-DRIFT)")
    print("=" * 70)
    ...
    def run_single_dialogue(dlg_idx: int, row: Dict[str, str]) -> str:
        ...
        drift_avoidance_labels = intent_clusters.get(target_intent, [])
        ...
        for turn_idx in range(1, args.max_turns + 1):
            ...
            print(f"  [Turn {turn_idx}/{args.max_turns}] User: '{user_text[:50]}...' -> Bot {conf_print}: '{assistant_text[:50]}...'")
